...
\] Indeed, we have Lemma \ref{affinetrans} that gives us this estimate. By a change of variables, the expected value of $X$ is represented by $M^3 \int_{u\asymp 1} R(u)^2 du$. This improves the bound on $S_3$ to\[
...
\begin{remark}
    The proof to Lemma \ref{affinetrans} does not use probability. Instead, it applies Plancherel's theorem on $X$ and bounds things in the Fourier domain.
\end{remark}
