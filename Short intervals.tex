...
We would like to answer the following question about primes in short intervals. Let $y\defeq y(x)$. What is the smallest asymptotic behavior of $y$
...
\end{remark}
\begin{theorem}[Littlewood \cite{Littlewood1922}] 
...
\end{theorem}
This following proof is expanded from a sketch of proof in Sagun Chanillo's lecture notes \cite{rutgernotes}.
\begin{proof}
    ...
    \end{align*}
    Where in the penulitimate step we made use of Littlewood's bound and exchanged the order of integration and summation.
...
\end{proof}
Theorem \ref{Hoheisel} gives the classical way to relate the distribution of primes in short intervals to the density of zeros away from the real-half line. The long-standing bound for zero density is due to separate proofs of Ingham and Huxley in 1940 and 1971 respectively:
\begin{theorem}[Ingham bound for zero density \cite{Ingham1940}]
    Let $1/2\leq \sigma\leq 3/4$. We have \[
        N(\sigma,t)\lesssim T^{\frac{3(1-\sigma)}{2-\sigma}}.
        \]
\end{theorem}
\begin{theorem}[Huxley bound for zero density \cite{Huxley1971}]
